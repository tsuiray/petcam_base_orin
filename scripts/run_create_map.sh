#!/usr/bin/env bash
# Launch create_map after ensuring agent binary exists.
set -e
...
if [[ ! -f "${MICROROS_WS}/install/local_setup.bash" ]]; then
  echo "ERROR: micro-ROS agent workspace missing: ${MICROROS_WS}"
...
if [[ ! -f "${REPO_ROOT}/install/local_setup.bash" ]]; then
  echo "ERROR: petcam workspace not built. Run: ${REPO_ROOT}/scripts/build_petcam_ws.sh"
  exit 1
fi
petcam_source "${REPO_ROOT}/install/local_setup.bash"

if ! ros2 pkg prefix micro_ros_agent >/dev/null 2>&1; then
  echo "ERROR: micro_ros_agent package not visible. Run: ./scripts/install_microros_agent.sh"
  exit 1
fi
...
echo "==> micro_ros_agent at: $(ros2 pkg prefix micro_ros_agent)"
echo "==> Launching create_map (watch for: micro_ros_agent starting / UDP 8888 OK)"
